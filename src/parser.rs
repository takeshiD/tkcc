...
use crate::types::{ExprEnum, Expression, Span, Statement, Statements};

/// Helper Parser
fn space_delimited<'a, O, E: ParseError<Span<'a>>, F: Parser<Span<'a>, Output = O, Error = E>>(
    f: F,
...
pub fn expr(input: Span) -> IResult<Span, Expression> {
    ...
    fold_many0(
        ...
        move || init.clone(),
        |acc, (op, val): (char, Expression)| match op {
            '+' => Expression::new(ExprEnum::Add(Box::new(acc), Box::new(val)), input),
            '-' => Expression::new(ExprEnum::Sub(Box::new(acc), Box::new(val)), input),
            _ => panic!("Binary operator must be '+' or '-'. Actualy got '{op}'"),
        },
    )
    .parse(r)
...
mod tests {
    ...
    fn test_expr() {
        ...
        assert!(expr(program).is_ok());
        let program = Span::new("-1");
        assert!(expr(program).is_ok());
        let program = Span::new("0");
        assert!(expr(program).is_ok());
